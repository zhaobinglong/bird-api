...
flowintime, s.user_name,s.user_code,s.team_name,s.bank_code,s.bank_name,s.company_name,s.sub_company_name from bird_order as o left join bird_seller as s on o.channel=s.identify_number and u.user_status!='0'  where o.policyno!='' order by o.flowintime desc";
		$res = $this->db->dql($sql);
		$data = array();
		while ($row = mysql_fetch_array($res, MYSQL_ASSOC)) {
			$row['insuredInfos'] = unserialize($row['insuredInfos']);
